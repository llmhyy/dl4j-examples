...
public class MLPClassifierLinear {
	...
	public static void main(String[] args) throws Exception {
		...
		MultiLayerConfiguration conf0 = new NeuralNetConfiguration.Builder()
				.seed(seed)
				.iterations(1)
		...
		MultiLayerNetwork model = new MultiLayerNetwork(conf0);
		model.init();
		...
		// rr.initialize(new FileSplit(new
		// ClassPathResource("/classification/train.csv").getFile()));
		// rr.reset();
		...
		// rrTest.initialize(new FileSplit(new
		// ClassPathResource("/classification/test.csv").getFile()));
		// rrTest.reset();
		// int nTestPoints = 500;
		// testIter = new RecordReaderDataSetIterator(rrTest,nTestPoints,0,2);
		// ds = testIter.next();
		// INDArray testPredicted = model.output(ds.getFeatures());
		// PlotUtil.plotTestData(ds.getFeatures(), ds.getLabels(),
